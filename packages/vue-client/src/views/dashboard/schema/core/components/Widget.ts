...
import { fallbackLabel, getPathVal, isRootNodePath, path2prop, setPathVal, validateFormDataAndTransformMsg } from './utils'
...
export default {
  ...
  setup(props: any, { emit }: any) {
    const genFormProvide = inject('genFormProvide') as any
    ...
    return () => {
      ...
      const miniDescriptionVNode = (miniDesModel && descriptionVNode)
        ? h(resolveComponent(COMPONENT_MAP.popover), {
            style: {
              margin: '0 2px',
              fontSize: '16px',
              cursor: 'pointer',
      ...
      return h(
        ...
        {
          // 错误只能显示一行，多余...
          error: (slotProps: any) => (slotProps.error
            ? h('div', {
                class: {
            ...
            ...props.widget ? [
              h( // 关键输入组件
                resolveComponent(props.widget),
                {
                  ...
                  'ref': widgetRef,
                  'onUpdate:modelValue': function updateModelValue(event: any) {
