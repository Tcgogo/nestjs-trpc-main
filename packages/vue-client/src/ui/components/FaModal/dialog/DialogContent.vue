...
<template>
  <DialogPortal>
    ...
    >
      <slot />
      <div class="absolute top-4 flex-center gap-2 inset-e-4">
        <button v-if="props.maximizable" class="rounded-sm bg-transparent opacity-70 ring-offset-background transition-opacity disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring hidden sm:inline-block" @click="handleMaximize">
