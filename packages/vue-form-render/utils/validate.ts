import { convertValue, validate } from './index'

// 值是是否为空
function isEmptyValue(value: any, schema: any) {
  // 多选组件的值为 [] 时，也判断为空值
...
function dealTypeValidate(key: any, value: any, schema: any = {}, _formData: any) {
  const checkList = []
  ...
  if (type === 'object') {
    const list = getValidateList(value, schema, _formData)
    checkList.push(...list)
  }
  else if (type === 'array') {
    value.forEach((v: any) => {
...
export function getValidateList(val: any = {}, schema: any = {}, formData?: any) {
  const _formData = formData || val
  const checkList: any[] = []
  const { properties, required } = schema
  // 校验必填（required 属性只在 type:object 下存在）
  if (required && required.length > 0) {
    required.forEach((key: any) => {
