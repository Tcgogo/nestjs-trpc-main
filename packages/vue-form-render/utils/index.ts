...
import { baseGet, hasRepeat, isEmail, isEmptyObject, isUrl } from './utils'
...
function getDefaultValue(schema: any) {
  ...
  if (isFunction(def)) {
    return defaultValue[type as keyof typeof defaultValue]
  }
  if (isFunction(enums)) {
    if (type === 'array') {
      return []
...
function resolve(schema: any, data?: any, options: any = {}) {
  const {
    // 类型
    type,
    // 对象子集
  ...
  if (type === 'object') {
    ...
    Object.keys(subs).forEach((name) => {
      const checkAndPass
        = checkRequired && ([] as any).concat(required).includes(name)
...
function getSubSchemas(schema: any = {}) {
  const {
    // object subset
    properties,
    // array subset
...
function validate({ name, schema, value, required = [] }: any) {
  const {
    type,
    'ui:options': options,
    message,
  ...
  if (required.includes(name) && (!value || !value.length)) {
    return '不能为空'
  }
  // 正则
  const usePattern = pattern && ['string', 'number'].includes(type)
  // 字符串
  if (type === 'string') {
    ...
    if (_finalValue && maxLength) {
      if (!isLength(_finalValue, { min: 0, max: Number.parseInt(maxLength, 10) })) {
    ...
    if (_finalValue && (minLength || minLength === 0)) {
      if (
        !_finalValue
        || !isLength(_finalValue, { min: Number.parseInt(minLength, 10), max: undefined })
      ) {
    ...
    if (format === 'image') {
      const imagePattern
        = '([/|.ws\\-])*.(?:jpg|gif|png|bmp|apng|webp|jpeg|json)' // eslint-disable-line
      // image 里也可以填写网络链接
  ...
  if (type === 'number') {
    ...
    if (maximum && Number.parseFloat(String(value)) > maximum) {
      return (message && message.maximum) || `数值不能大于 ${maximum}`
    }
    if ((minimum || minimum === 0) && Number.parseFloat(String(value)) < minimum) {
...
function safeEval(code: any) {
  return new Function(`"use strict"; ${code}`)()
}

function evaluateString(string: any, formData: any, rootValue: any) {
  return safeEval(`
...
function convertValue(item: any, formData: any, rootValue: any) {
